...
function loadPlant(plantname) {
	...
	$.ajax({
		...
		success: function (data) {
			if (data == 1) {
				document.getElementById('plantinfodiv').innerHTML="<div style='position:relative;'>\
				<h3>This Plant IS READY for harvest!</h3></div>\
				<div style='position:relative;right:20px;top:20px;margin-left:auto;'><a href='#'onclick=\"javascript:document.getElementById('plantinfodiv').style.display='none';\">CLOSE</a></div>"+document.getElementById('plantinfodiv').innerHTML;
			}
			else {
